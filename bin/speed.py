...
class Allocate(SpeedCLI, help='Speed tune the specified characters using the specified rune levels'):
    ...
    def main(self):
        party = Party(
            [PartyMember(self.mm_session.mb.get_character(char_info)) for char_info in self.characters],
            deltas=self.deltas,
        )
    ...
    def get_members(self):
        ...
        for char_info in self.characters:
            try:
                ...
                members.append(PartyMember(self.mm_session.mb.get_character(char_info)))
            else:
                levels = list(map(int, map(str.strip, levels.split(','))))
                members.append(PartyMember(self.mm_session.mb.get_character(name_or_id), levels))
